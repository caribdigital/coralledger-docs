...
function HomepageHeader() {
    ...
    const complyUrl = siteConfig.customFields?.complyUrl;
    return (<header className={clsx('hero hero--primary', styles.heroBanner)}>
      <div className="container">
        ...
        <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '1.1rem', marginBottom: '1rem' }}>
          Free beta — all features included during open beta
        </p>
        <div className={styles.buttons}>
          <Link className="button button--secondary button--lg" to="/docs/getting-started">
            Get Started
          </Link>
          <Link className="button button--outline button--lg" style={{ marginLeft: '1rem', color: 'white', borderColor: 'white' }} to={complyUrl}>
            Open Comply App
          </Link>
        </div>
      </div>
...
function QuickLinks() {
    return (<section className={styles.quickLinks}>
      <div className="container">
        <div className="row">
...
export default function Home() {
    const { siteConfig } = useDocusaurusContext();
    return (<Layout title="VAT Compliance Documentation" description="Documentation for CoralLedger Comply - VAT compliance for Bahamian businesses">
      <HomepageHeader />
      <main>
        <QuickLinks />
        <HomepageFeatures />
