...
- **Not an installable PWA.** A web manifest is present, but there is no service worker — so Comply cannot be installed as a standalone app with background push notifications. Browser-only.
