---
sidebar_position: 1
title: By Statute
description: Statute-first VAT reference pages mapped to CoralLedger Comply workflows
---

# By Statute

This section provides a statute-first navigation path for teams that review VAT obligations from legal text before moving into product workflows.

Each page follows the same structure:

1. **What statute says** — Practical summary of the legal requirement
2. **What platform does** — How CoralLedger Comply supports the requirement
3. **Customer responsibility** — What your team must still do

:::info
This section is a practical product guide, not legal advice. Use it alongside official Department of Inland Revenue materials and your VAT advisor.
:::

## Statute Reference Pages

- [Registration obligation and threshold](/docs/statutes/registration-obligation-threshold)
- [Tax invoices and credit notes](/docs/statutes/tax-invoices-credit-notes)
- [Time of supply and period assignment](/docs/statutes/time-of-supply-period-assignment)
- [Output tax calculation and declaration](/docs/statutes/output-tax-calculation-declaration)
- [Input tax deduction eligibility](/docs/statutes/input-tax-deduction-eligibility)
- [Partial exemption and apportionment](/docs/statutes/partial-exemption-apportionment)
- [Bad debt relief](/docs/statutes/bad-debt-relief)
- [Filing and payment deadlines](/docs/statutes/filing-payment-deadlines)
- [Record keeping and retention](/docs/statutes/record-keeping-retention)
- [Assessments, interest, and penalties](/docs/statutes/assessments-interest-penalties)
- [Audit and information powers](/docs/statutes/audit-information-powers)
- [Refunds and repayments](/docs/statutes/refunds-repayments)

## Navigate by Topic

Prefer product-first documentation? Use the **By Topic** section in the sidebar to navigate feature guides such as Transactions, VAT Returns, Compliance, Audit Trail, and Data Operations.
