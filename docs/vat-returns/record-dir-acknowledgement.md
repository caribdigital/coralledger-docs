...
|---|---|---|
| **DIR Online Channel** | You lodged the return through a current DIR online channel. | Record the transaction reference number if one is provided. |
...
For payable returns (Net VAT due > 0), the lifecycle does not end at `Lodged` - it ends at `Lodged & Paid`. You record the payment back into Comply after you have paid the DIR through a current payment channel.
