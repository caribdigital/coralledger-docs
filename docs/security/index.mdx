...
### IP Blocking
Administrators can manually block suspicious IPs. Automatic blocking after repeated failed login attempts is not surfaced in the current Comply codepath.
...
### Kill Switch
Emergency control to immediately disable fraud detection during security incidents. It is not a platform-wide app kill switch and does not lock individual accounts from this UI.
...
- Append-only audit entries with hash-chain verification; database-level WORM depends on operator configuration
